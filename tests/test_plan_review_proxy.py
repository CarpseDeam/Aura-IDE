"""PlanReviewProxy: worker-thread <-> GUI synchronization for one active review."""
...
def _wait_for_pending(proxy: PlanReviewProxy, timeout: float = 2.0) -> None:
    ...
    while proxy._pending is None and time.time() < deadline:
        time.sleep(0.01)
    assert proxy._pending is not None, "worker thread never registered its pending review"


def test_stop_cancels_a_pending_review_and_unblocks_the_worker_thread(qapp) -> None:
    ...
    assert not t.is_alive(), "Stop must unblock the worker thread, never strand it"
    assert result["decision"].approved is False


def test_resolve_approved_delivers_the_edited_plan_to_the_worker_thread(qapp) -> None:
    proxy = PlanReviewProxy()
    result: dict = {}
...
def test_review_requested_signal_carries_the_review_id(qapp) -> None:
    ...
    # The signal crosses threads (worker -> GUI), so delivery is queued and
