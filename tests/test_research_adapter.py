"""Tests for the research adapter and native provider web search seam.

The legacy browser/Drone web-research path is retired.  The adapter now
resolves research requests through the provider's native Responses API
web search (DeepSeek) and returns explicit unsupported results for every
other provider — the old browser/Drone system is never launched.
"""
...
def test_execute_web_research_request_uses_native_search(tmp_path, monkeypatch):
    ...
    def fake_native(*, provider, question, context=None, model=None, cancel_event=None):
        captured.update(provider=provider, question=question, context=context)
...
def test_execute_web_research_request_unsupported_provider(tmp_path):
    """Providers without native search get a clear unsupported result."""
    request = build_research_request("Are there any World Cup matches today?")
    result = execute_web_research_request(tmp_path, request, provider="openai")
...
def test_execute_web_research_request_never_launches_drone(tmp_path, monkeypatch):
    ...
    result = execute_web_research_request(
        tmp_path, request, provider="openai"
    )
