...
function RunsScreen() {
  ...
  const [runs, setRuns] = useState<DroneRun[]>([]);
  ...
  // Phase-gated fetch — only send drone.status and drone.list_recent after verified
  useEffect(() => {
    ...
    const unsubStatus = socket.on('drone.status_result', (msg: any) => {
      if (msg.payload?.error) {
        setFetchError(msg.payload.error);
        setLoading(false);
        return;
      }
      setDroneStatus(msg.payload ?? null);
    });

    const unsubList = socket.on('drone.list_result', (msg: any) => {
      if (msg.payload?.error) {
        setFetchError(msg.payload.error);
        setLoading(false);
        return;
      }
      setRuns(msg.payload?.runs ?? []);
      setLoading(false);
    });

    socket.send('drone.status', {}, desktopId);
    socket.send('drone.list_recent', {}, desktopId);
  ...
  function kindBadge(kind: string) {
    const label =
      kind === 'drone' ? 'Drone' : kind === 'worker' ? 'Worker' : kind;
    return (
      <span
        style={{
          fontSize: '0.65rem',
  ...
  return (
    ...
    >
      ...
      <main style={{ flex: 1, overflow: 'auto', padding: '0.25rem 0 1rem' }}>
        ...
        {/* Empty state */}
        {!loading && !fetchError && runs.length === 0 && !droneStatus?.running && (
          <div
            style={{
              textAlign: 'center',
              marginTop: '2rem',
        ...
        {!loading && (
          <>
            ...
            >
              ...
              {droneStatus?.running && droneStatus.run ? (
                <div>
                  <div
                    style={{
                      display: 'flex',
                  ...
                  >
                    <div style={{ fontWeight: 600, fontSize: '0.95rem' }}>
                      {droneStatus.run.label}
                    </div>
                    {kindBadge(droneStatus.run.kind)}
                  </div>
                  <div
                    style={{
                      display: 'flex',
                  ...
                  >
                    <span
                      style={{
                        ...
                        height: 8,
                        borderRadius: '50%',
                        background: statusColor(droneStatus.run.status),
                        flexShrink: 0,
                      }}
                    />
                    <span style={{ fontSize: '0.82rem', color: tokens.fgDim }}>
                      {statusLabel(droneStatus.run.status)}
                    </span>
                    <span
                      style={{
                        fontSize: '0.72rem',
                    ...
                    >
                      Started {formatDate(droneStatus.run.started_at)}
                    </span>
                  </div>
                </div>
              ) : (
