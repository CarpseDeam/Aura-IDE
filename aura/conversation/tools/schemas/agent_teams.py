...
def build_run_agent_team_tool_def(
    ...
    return {
        ...
        "function": {
            "name": "run_agent_team",
            "description": (
                "Use Aura directly for everyday work. Call this only when "
                "separating the task into specialist roles would genuinely "
                "improve the outcome. Assemble one temporary team of Agents, "
                "run it, and wait for one Aura Result. A handoff sends work to the next "
                "specialist. Several outgoing handoffs branch; several incoming "
                "handoffs wait and join. Reuse one Agent in multiple occurrences "
                "when it has several assignments. A helper is optional and may "
                "have helpers of its own. Read / Write specialists may work "
                "together under Aura's existing isolation and ordering rules. "
