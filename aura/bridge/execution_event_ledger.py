...
touched/created/edited/deleted file ledgers, and EXECUTION_FILE_CHANGED
bus event emission.
...
from aura.events import EXECUTION_FILE_CHANGED


class EventRelayExecutionLedger:
    """Owns the execution-ledger state for ExecutionEventRelay.

    Handles applied file mutations, not-applied write attempts, read-before-edit
    tracking, and EXECUTION_FILE_CHANGED bus event emission.

    Exposes mutable public attributes so ExecutionEventRelay can delegate to them
    ...
    def handle_tool_result(
        ...
        code in ExecutionEventRelay.relay() to preserve behavior exactly.
        """
        # ------------------------------------------------------------------
