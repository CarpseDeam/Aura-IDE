...
def _format_spec_as_user_message(task: WorkerTaskSpec | WorkerDispatchRequest) -> str:
    ...
    parts.extend([
        ...
        "- Large files and scoped targets use a bounded read_file (offset and limit) around the edit region.",
        "- Use the returned content_hash from the latest successful read_file result as expected_file_hash on patch_file.",
        "- If read_file returns truncated: true, treat it as navigation context only; read the actual edit region with a further bounded read_file (offset and limit) and use that read's content_hash before patch_file.",
        ...
        "- Use grep_search for discovery; use read_file for exact known-file verification.",
