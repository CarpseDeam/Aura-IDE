...
def build_drone_architect_prompt(
    ...
    lines.append("- The Drone is always a folder-backed Python worker, not an MCP wrapper.")
    ...
    lines.append("3. Read the existing Drone folder files (drone.json, main.py, smoke.py,")
    ...
    lines.append("5. Run the Drone smoke check.")
    lines.append("6. Call register_drone_folder with the workspace-relative folder path to")
    lines.append("   re-register the repaired Drone. This overwrites the existing registration")
    lines.append("   with the repaired version.")
    lines.append("7. Report that the existing Drone was repaired and re-registered, including")
    lines.append("   what was changed and the smoke check result.")
    lines.append("")
    if build_brief:
