"""Task Checklist snapshot renderer."""
...
from aura.gui.theme import BG_ALT, BORDER, FG, FG_DIM, FG_MUTED, SUCCESS, WARN
...
class TaskChecklistWidget(QFrame):
    """Passive renderer for full Task Checklist snapshots."""
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.setStyleSheet(
            ...
            QLabel#taskChecklistTitle {{
                color: {FG_DIM};
                font-size: 11px;
                font-weight: 700;
                padding: 0;
            }}
            QLabel#taskChecklistText {{
                color: {FG};
                font-size: 12px;
                padding: 0;
            }}
            QLabel#taskChecklistIcon {{
                font-size: 13px;
                font-weight: 700;
                padding: 0;
            }}
        ...
        self._title = QLabel("TASK CHECKLIST", self)
        self._title.setObjectName("taskChecklistTitle")
    ...
    def update_snapshot(self, items: list[dict[str, str]]) -> None:
        ...
        total = len(items)
        self._title.setText(f"TASK CHECKLIST \u00b7 {done_count}/{total}")
    ...
    def clear(self) -> None:
        _log.info("DIAGNOSTIC TaskChecklistWidget.clear called row_count=%d", len(self._rows))
